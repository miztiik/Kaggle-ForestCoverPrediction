...
## Author:Miztiik
## Source Inspiration : https://www.kaggle.com/users/116664/apurva-dubey/forest-cover-type-prediction/r-starter-code
## Date : 16May2015 01:06
###################################################
...
#Find variables with zero variance and remove them
var0 <- unlist(lapply(train, function(x) 0 == var(if (is.factor(x)) as.integer(x) else x)))
#table(var0)
#which(var0==TRUE)
...
# caret likes the factor variables to be non numeric
...
#write.csv(submission, "../input/submission.csv", row.names=FALSE)
